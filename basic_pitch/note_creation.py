...
def sonify_midi(
    ...
    # convert the floating point audio to int16 for wider compatibility
    if y.dtype.kind == "f":
        y_int16 = np.int16(np.clip(y, -1.0, 1.0) * np.iinfo(np.int16).max)
    else:
        y_int16 = y
    wavfile.write(save_path, sr, y_int16)
